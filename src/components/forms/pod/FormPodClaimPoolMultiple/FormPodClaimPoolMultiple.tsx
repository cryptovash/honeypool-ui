 ...
 * @return Component
 */
export const FormPodWithdrawToMultiple = ({ label, defaultToken, defaultValues }: IPodForm) => {
  /* ----------------------- */
  /* --- Component State --- */
  /* ----------------------- */
  ...
  const onSubmit = async (values) => {
    if (values) {
      claimAmountSet(commifyTokenBalance(utils.parseEther(formValues.poolAmount).toString()))
      send(account)
    }
  }
  ...
  if (idx(state, (_) => _.status) == 'Mining') {
    return (
      <TransactionMining state={state} amount={claimAmount} action='Withdrawing' symbol={'POOL'} />
    )
  }
