...
export const FormPodDepositToMultiple = ({ label, defaultToken, defaultValues }: IPodForm) => {
  ...
  /* --- Form State --- */
  /* ------------------ */
  const { handleSubmit, register, control, setValue, watch, trigger, formState } = useForm({
    defaultValues: {
      pod: selectOptions[0]
    }
  })
  ...
  /* --- Component Hooks --- */
  /* ----------------------- */
  const [userSharesToUnderlyingPercentage, userSharesToUnderlyingPercentageSet] = useState()
  useEffect(() => {
    if (isPositiveBigNumber(podTokenBalance) && isPositiveBigNumber(userUnderlyingBalance)) {
      const s = podTokenBalance.mul(CONSTANT_ROUNDING).div(userUnderlyingBalance)
  ...
  const onSubmit = async (values) => {
    ...
    const shareAmount = tokenAmount.mul(userSharesToUnderlyingPercentage).div(CONSTANT_ROUNDING)
  ...
  const setInputAmountMax = () => {
    setValue('tokenAmount', utils.formatUnits(userUnderlyingBalance, decimals), {
      shouldDirty: true,
      shouldValidate: true
    })
    ...
    if (idx(formValues, (_) => _.tokenAmount)) {
      ;(async () => {
        try {
          ...
          const getEarlyExitFee = await contract.callStatic.getEarlyExitFee(tokenAmount)
  ...
  return (
    <>
      ...
      <form className={'form-default text-white'} onSubmit={handleSubmit(onSubmit)}>
        <input
          hidden
        ...
        <SelectTokenWithAmountInput
          ...
          classNameInput='bg-yellow-600 bg-opacity-0 text-xl font-light text-white h-full w-full focus:outline-none'
          classNameInputContainer='bg-yellow-600 bg-opacity-20 h-15 flex items-center justify-between input-skinny relative'
        ...
        <WalletIsConnected>
          <button className='btn btn-teal w-full'>{label}</button>
        </WalletIsConnected>
        {earlyExitFeeError && (
          <>
            <Spacer className='my-1' />
            <span className='inline-block text-red-500 mx-auto'>{earlyExitFeeError}</span>
          </>
        )}
        <Spacer className='my-5' />
        <div className='text-center'>
          <span className='text-yellow-600 text-sm font-normal mx-auto'>
            <span className='text-md'>⚠️</span> Withdrawing everything will make you ineligible to
            receive your share of hive winnings.
          </span>
        </div>
        <Spacer className='my-5' />
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-5'>
          <div className='bg-yellow-500 bg-opacity-30 rounded-md pt-4 pb-2 text-center'>
            <span className='block text-normal text-white text-sm lg:text-xl'>Pod's Float</span>
            <ERC20Balance
          ...
          <div className='bg-yellow-500 bg-opacity-30 rounded-md pt-4 pb-2 text-center'>
            <span className='block text-normal text-white text-sm lg:text-xl'>
              Your early exit fee:
            </span>
            <h4 className='text-3xl lg:text-4xl text-white'>
              {isExitFeeCalculated ? (
                <TokenBalance
                  className='mr-1'
